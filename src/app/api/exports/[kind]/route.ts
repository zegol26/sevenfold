...
async function rowsForKind(kind: string): Promise<RecordMap[]> {
  ...
  if (kind === "site-handler") return (await getProjectExecutionRegistry()).sites as unknown as RecordMap[];
  if (kind === "governance-report") {
    const governance = await getDeliveryGovernanceRegistry();
    return [...governance.governanceRecords, ...governance.changeRequests, ...governance.incidents] as unknown as RecordMap[];
  }
  if (kind === "talent-planning") return (await getTalentPlanningRegistry()).talents as unknown as RecordMap[];
  if (kind === "ratecard") return (await getRatecardRegistry()).resources as unknown as RecordMap[];
  return safeOpportunityRows(kind);
}

async function safeOpportunityRows(kind: string): Promise<RecordMap[]> {
  try {
    if (kind === "opportunity-summary") {
      const rows = await getDb().opportunity.findMany({ orderBy: { createdAt: "desc" }, take: 500 });
    ...
    if (kind === "risk-register") {
      const rows = await getDb().riskRegister.findMany({ include: { opportunity: true }, orderBy: { createdAt: "desc" }, take: 1000 });
    ...
    if (kind === "cashflow-summary") {
      const rows = await getDb().cashflowOption.findMany({ include: { opportunity: true }, orderBy: { createdAt: "desc" }, take: 500 });
    ...
    if (kind === "sds-summary") {
      const rows = await getDb().salesDecisionSubmission.findMany({ include: { opportunity: true }, orderBy: { createdAt: "desc" }, take: 500 });
    ...
    if (kind === "sdoa-delta") {
      const rows = await getDb().contractDeviation.findMany({ include: { orderAcknowledgement: { include: { opportunity: true } } }, orderBy: { createdAt: "desc" }, take: 1000 });
      return rows.map((row) => ({
